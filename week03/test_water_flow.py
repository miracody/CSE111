...
def test_pressure_loss_from_pipe_reduction():
    ...
    assert pressure_loss_from_pipe_reduction(0.28687, 1.65, 471729, 0.048692) == approx(-327.488, abs=0.001)
    assert pressure_loss_from_pipe_reduction(0.28687, 1.75, 500318, 0.048692) == approx(-368.364, abs=0.001)

def test_convert_kilopascals_to_psi():
    assert convert_kilopascals_to_psi(0.0) == approx(0.000, abs=0.001)
    assert convert_kilopascals_to_psi(101.325) == approx(14.70, abs=0.01)
    assert convert_kilopascals_to_psi(200.0) == approx(29.01, abs=0.01)
    assert convert_kilopascals_to_psi(500.0) == approx(72.52, abs=0.01)
