# Enhancement: Added element name display, error handling, repeat loop, and formatted output.
...
def display_element_details(symbol_quantity_list, periodic_table_dict):
    print("\n🧪 Element Breakdown:")
    print(f"{'Symbol':<6} {'Name':<15} {'Qty':<5} {'Atomic Mass':<12}")
    print("-" * 40)
    for symbol, quantity in symbol_quantity_list:
        name, atomic_mass = periodic_table_dict[symbol]
        print(f"{symbol:<6} {name:<15} {quantity:<5} {atomic_mass:<12.5f}")
    print("-" * 40)
...
def main():
    ...
    while True:
        try:
            formula = input("\nEnter the molecular formula of the sample: ").strip()
        ...
        display_element_details(symbol_quantity_list, periodic_table)

        print(f"\n📊 Results:")
        print(f"Molar Mass     : {molar_mass:.5f} grams/mole")
        print(f"Sample Mass    : {sample_mass:.5f} grams")
        print(f"Number of Moles: {number_of_moles:.5f} moles")

        again = input("\nWould you like to calculate another compound? (yes/no): ").strip().lower()
        if again != "yes":
            print("👋 Goodbye! Stay curious, chemist-in-training.")
            break
